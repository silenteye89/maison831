    ...
    <section className="py-[100px] md:py-[120px] px-6 md:px-10">
      <div className="max-w-[1080px] mx-auto">
        <div className="text-center mb-10">
          <FadeIn>
            <a href="https://instagram.com/maison831.kr" target="_blank" rel="noopener noreferrer" className="text-[12px] tracking-[0.15em] uppercase text-oak/35 hover:text-brass transition-colors duration-300">@maison831.kr</a>
          </FadeIn>
        </div>
        <div className="grid grid-cols-3 md:grid-cols-6 gap-1.5 md:gap-2">
          {photos.map((p, i) => (
            <ImageReveal key={i} delay={i * 0.04}>
              <motion.a href="https://instagram.com/maison831.kr" target="_blank" rel="noopener noreferrer" className="relative aspect-square overflow-hidden group block">
                <div className="absolute inset-0 bg-cover bg-center transition-transform duration-700 group-hover:scale-105" style={{ backgroundImage: `url('${p.src}')`, backgroundColor: p.color }} />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-400 flex items-center justify-center">
                  <svg className="w-4 h-4 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-400" fill="currentColor" viewBox="0 0 24 24">
